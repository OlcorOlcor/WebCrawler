(function () {
    ...
    function instance$1($$self, $$props, $$invalidate) {
    	...
    	const simulation$1 = simulation(nodes).force("link", link(links).id(d => d.id)).force("charge", manyBody()).force("center", center(width / 2, height / 2)).on("tick", ticked);

    	// Create the SVG container.
    	const svg = create$1("svg").attr("width", width).attr("height", height).attr("viewBox", [0, 0, width, height]).attr("style", "max-width: 100%; height: auto;");

    	// Add a line for each link, and a circle for each node.
    	const link$1 = svg.append("g").attr("stroke", "#999").attr("stroke-opacity", 0.6).selectAll().data(links).join("line").attr("stroke-width", d => Math.sqrt(d.value));
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	const groupColour = ordinal(category10);
    	let simulation$1, context;

    	onMount(() => {
    		context = canvas.getContext('2d');

    		simulation$1 = simulation(nodes).force("link", link(links).id(d => d.id)).force("charge", manyBody()).force("center", center(width / 2, height / 2)).on("tick", () => {
    		...
    		select(context.canvas).on("mousemove", () => {
    			const d = simulation$1.find(undefined, undefined, nodeRadius);

    			if (d) {
    				console.log(undefined, undefined, 'title: ', d.id, ' ', d.x, d.y);
    		...
    		select(canvas).call(drag().container(canvas).subject(dragsubject).on("start", dragstarted).on("drag", dragged).on("end", dragended));
    	});

    	// Use the d3-force simulation to locate the node
    	function dragsubject() {
    		return simulation$1.find(undefined, undefined, nodeRadius);
    	}

    	function dragstarted() {
    		if (!undefined) simulation$1.alphaTarget(0.3).restart();
    		undefined = undefined;
    		undefined = undefined;
    	}
    	...
    	function dragended() {
    		if (!undefined) simulation$1.alphaTarget(0);
    		undefined = null;
    		undefined = null;
    	}
    	...
    	linear().domain([0, 20]).range([padding.left, width - padding.right]);
    	linear().domain([0, 12]).range([height - padding.bottom, padding.top]);
    	linear().domain([0, height]).range([height, 0]);
